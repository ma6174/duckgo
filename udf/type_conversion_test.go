...
func TestGoTypeToDuckDBTypeInfo(t *testing.T) {
	...
	tests := []struct {
		...
		{"int", reflect.TypeFor[int](), duckdb.TYPE_INTEGER, false, ""},
		{"int64", reflect.TypeFor[int64](), duckdb.TYPE_BIGINT, false, ""},
		{"float32", reflect.TypeFor[float32](), duckdb.TYPE_FLOAT, false, ""},
		{"float64", reflect.TypeFor[float64](), duckdb.TYPE_DOUBLE, false, ""},
		{"string", reflect.TypeFor[string](), duckdb.TYPE_VARCHAR, false, ""},
		{"bool", reflect.TypeFor[bool](), duckdb.TYPE_BOOLEAN, false, ""},
		{"[]byte", reflect.TypeFor[[]byte](), duckdb.TYPE_BLOB, false, ""},
		{"time.Time", reflect.TypeFor[time.Time](), duckdb.TYPE_TIMESTAMP, false, ""},

		// Struct tests
		{"SimpleStruct", reflect.TypeFor[SimpleStruct](), duckdb.TYPE_STRUCT, false, ""},
		{"NestedStruct", reflect.TypeFor[NestedStruct](), duckdb.TYPE_STRUCT, false, ""},
		{"PointerToSimpleStruct", reflect.TypeFor[*SimpleStruct](), duckdb.TYPE_STRUCT, false, ""}, // Test with pointer to struct
		{"StructWithUnexportedFields", reflect.TypeFor[StructWithUnexported](), duckdb.TYPE_STRUCT, false, ""},
		{"EmptyStruct (error)", reflect.TypeFor[EmptyStruct](), 0, true, "no exported fields"},

		// Map tests
		{"map[string]int32", reflect.TypeFor[map[string]int32](), duckdb.TYPE_MAP, false, ""},
		{"map[int16]SimpleStruct", reflect.TypeFor[map[int16]SimpleStruct](), duckdb.TYPE_MAP, false, ""},
		{"map[string]*SimpleStruct", reflect.TypeFor[map[string]*SimpleStruct](), duckdb.TYPE_MAP, false, ""}, // Map with pointer to struct values
		// Test a map where the key is a struct (DuckDB might have restrictions on this, but our type conversion should attempt it)
		{"map[MapKeyStruct]string", reflect.TypeFor[map[MapKeyStruct]string](), duckdb.TYPE_MAP, false, ""},
		{"map[string]map[int]string", reflect.TypeFor[map[string]map[int]string](), duckdb.TYPE_MAP, false, ""}, // Nested map

		// Negative tests for basic types
		{"unsupported chan", reflect.TypeFor[chan int](), 0, true, "unsupported Go type kind for UDF: chan (specific type: chan int)"},
		{"unsupported func", reflect.TypeFor[func()](), 0, true, "unsupported Go type kind for UDF: func (specific type: func())"},
		{"unsupported slice []int", reflect.TypeFor[[]int](), 0, true, "unsupported Go slice element type"},
	}
...
func TestConvertToReflectValue(t *testing.T) {
	...
	tests := []struct {
		...
		{"nil to interface", nil, reflect.TypeFor[any](), nil, false, ""},
		{"int64 to int", int64(123), reflect.TypeFor[int](), int(123), false, ""},
		{"float64 to float32", float64(123.45), reflect.TypeFor[float32](), float32(123.45), false, ""},
		{"string to string", "hello", reflect.TypeFor[string](), "hello", false, ""},
		{"bool to bool", true, reflect.TypeFor[bool](), true, false, ""},
		{"[]byte to []byte", []byte("blob"), reflect.TypeFor[[]byte](), []byte("blob"), false, ""},
		{
		...
		{"nil to int (error)", nil, reflect.TypeFor[int](), nil, true, "cannot convert nil (driver.Value) to Go type int"},
		{"string to int (error)", "not-an-int", reflect.TypeFor[int](), nil, true, "cannot convert DuckDB driver.Value"},
		{"int64 to string (error - disallowed implicit)", int64(65), reflect.TypeFor[string](), nil, true, "cannot convert DuckDB driver.Value"},

		// Struct conversion tests
		{
			"map to SimpleStruct (success)",
			map[string]any{"I": int32(10), "S": "test", "B": true},
			reflect.TypeFor[TestSimpleStruct](),
		...
		},
		{
			...
			reflect.TypeFor[TestNestedStruct](),
		...
		},
		{
			...
			reflect.TypeFor[map[int]TestSimpleStruct](),
		...
		},
		{
			...
			reflect.TypeFor[map[string]int](),
			nil,
			true, "expected duckdb.Map or duckdb.OrderedMap from driver",
		},
	}
